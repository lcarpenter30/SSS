...
def get_file_names():
    ...
    for key in file_db.keys() :
        file_info = file_db[key]
        if search =="ALL":
            data['files'].append({
                'FILENAME': str(file_info.filename),
                'OWNER': str(file_info.uid),
                'DID': str(file_info.fid)
            })
        else:
            if search in file_info.filename :
                data['files'].append({
                    'FILENAME': str(file_info.filename),
                    'OWNER': str(file_info.uid),
                    'DID': str(file_info.fid)
    ...
    if addpermissionID == "ALL" :
        ...
        for key in users_db.keys():
            if key != user_name:
                ...
                print "DELEGATED USER:" + str(key) + "\nWITH PERMISSIONS:" + str(file_info.rights[
                    key]) + "\nFOR: " + str(addtime) + "s\n"

    else:
        if addpermissionID in users_db.keys():
            #fmt=map(int,addtime.split(','))
            time= datetime.datetime.now()
            accessDeadline=time + datetime.timedelta(seconds=fmt)
            ...
            file_db[filename] = file_info
            print "DELEGATED USER:"+str(addpermissionID)+"\nWITH PERMISSIONS:"+ str(file_info.rights[addpermissionID])+"\nFOR: "+str(addtime) +"s"
        else:
